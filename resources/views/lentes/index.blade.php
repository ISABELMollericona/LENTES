...
@section('title', 'Catálogo de Lentes')
        ...
        <div class="row align-items-center">
            <div class="col-lg-6">
                ...
                <p class="text-gold fw-semibold" style="font-size: 1.1rem;">
                    <i class="bi bi-search me-2"></i>{{ $lentes->total() }} lentes disponibles
                </p>
            </div>
...
<div class="container py-5">
    ...
    <div class="mb-5">
        <div class="d-flex justify-content-between align-items-center mb-4 pb-3" style="border-bottom: 2px solid rgba(212, 175, 55, 0.3);">
            <div>
                ...
                <p class="text-muted mb-0 small">
                    <i class="bi bi-check-circle me-1" style="color: #D4AF37;"></i>{{ $lentes->total() }} resultados encontrados
                </p>
            </div>
            <div class="d-flex gap-2">
                <button class="btn btn-outline-gold d-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#filtrosModal">
                    ...
                    <span class="badge bg-gold" style="background: #D4AF37; color: #1a1a1a; font-size: 0.75rem;">{{ count(request()->query()) }}</span>
                    @endif
                </button>
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-dark" style="background: #1a1a1a; border-color: rgba(212, 175, 55, 0.2);">
                        <li><a class="dropdown-item" href="{{ request()->fullUrlWithQuery(['sort' => 'reciente']) }}" style="color: #e0e0e0;">Más recientes</a></li>
                        <li><a class="dropdown-item" href="{{ request()->fullUrlWithQuery(['sort' => 'precio_asc']) }}" style="color: #e0e0e0;">Menor precio</a></li>
                        <li><a class="dropdown-item" href="{{ request()->fullUrlWithQuery(['sort' => 'precio_desc']) }}" style="color: #e0e0e0;">Mayor precio</a></li>
                        <li><a class="dropdown-item" href="{{ request()->fullUrlWithQuery(['sort' => 'nombre']) }}" style="color: #e0e0e0;">A-Z</a></li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex flex-wrap gap-2 mb-4">
            ...
            @foreach(['RAY-BAN', 'TOM FORD', 'GUCCI', 'PERSOL', 'OAKLEY', 'SAINT LAURENT', 'PRADA', 'DIOR'] as $nombreMarca)
            @php
                $marcaActiva = strtoupper(request('marca', ''));
                $esActiva = $marcaActiva === strtoupper($nombreMarca);
            @endphp
            <a href="{{ $esActiva ? route('catalogo.index') : '?marca='.urlencode($nombreMarca) }}"
               class="btn btn-sm {{ $esActiva ? 'btn-gold' : 'btn-outline-gold' }}"
               style="border-radius: 25px; font-size: 0.85rem;">
                @if($esActiva)<i class="bi bi-x-circle me-1"></i>@endif
                {{ $nombreMarca }}
            </a>
            @endforeach
        </div>
    ...
    <div class="row">
        <div class="col-12">
                ...
                <div class="col-sm-6 col-lg-4 col-xl-3 scroll-reveal">
                    <div class="card card-producto h-100 shadow-sm">
                        <div class="position-relative overflow-hidden" style="aspect-ratio: 4/3; background: #ffffff;">
                             <img src="{{ $lente->imagen_url ?? 'https://via.placeholder.com/300x300?text=Sin+Imagen' }}"
                                 class="card-img-top w-100 h-100" alt="{{ $lente->nombre }}"
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <p class="text-muted text-uppercase" style="font-size: 0.8rem; letter-spacing: 1px; margin-bottom: 0.5rem;">
                                <i class="bi bi-tag me-1"></i>{{ $lente->marca->nombre ?? 'Sin marca' }}
                            </p>
                            ...
                            <p class="card-text small flex-grow-1" style="color: #999999; margin-bottom: 0.75rem;">
                                <i class="bi bi-bag me-1"></i>{{ $lente->categoria->nombre ?? 'N/A' }} •
                                <i class="bi bi-person me-1"></i>{{ ucfirst($lente->genero) }}
                            </p>
                            ...
                            <div class="d-flex justify-content-between align-items-end mb-3">
                                <div>
                                    ...
                                    <span class="precio">Bs {{ number_format($lente->precio, 2) }}</span>
                                </div>
                            </div>
                            ...
                            @if($lente->estado == 'disponible')
                                @auth
                                <form action="{{ route('carrito.agregar', $lente) }}" method="POST" class="d-inline w-100 ajax-add-cart">
                                    @csrf
                                ...
                                @else
                                <a href="{{ route('login') }}" class="btn btn-outline-gold w-100 fw-semibold py-2" style="border-radius: 8px;">
                            ...
                            <a href="{{ route('catalogo.show', $lente) }}" class="stretched-link"></a>
                        </div>
                    </div>
                </div>
                @endforeach
            ...
            <div class="d-flex justify-content-center mt-5">
                {{ $lentes->withQueryString()->links('partials.pagination') }}
            </div>
            @else
            <!-- Sin resultados -->
    ...
    <div class="modal-dialog modal-dialog-scrollable">
        <div class="modal-content modal-gold" style="background: linear-gradient(135deg, rgba(26, 26, 26, 0.95), rgba(15, 15, 15, 0.95)); border: 1px solid rgba(212, 175, 55, 0.2);">
            ...
            <div class="modal-body" style="color: #e0e0e0;">
                <form method="GET" action="{{ route('catalogo.index') }}" id="form-filtros">
                    <!-- Búsqueda -->
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="search" placeholder="Buscar lentes..."
                               value="{{ request('search') }}" style="background: rgba(26, 26, 26, 0.6); border-color: rgba(212, 175, 55, 0.2);">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex flex-column gap-2">
                            @foreach(['todos' => 'Todos', 'hombre' => 'Hombre', 'mujer' => 'Mujer', 'unisex' => 'Unisex'] as $val => $label)
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="genero" id="gen_{{ $val }}"
                                       value="{{ $val == 'todos' ? '' : $val }}" 
                                       {{ (request('genero') == ($val == 'todos' ? '' : $val)) ? 'checked' : '' }}>
                                <label class="form-check-label" for="gen_{{ $val }}">{{ $label }}</label>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="tipo_montura" style="background: rgba(26, 26, 26, 0.6); border-color: rgba(212, 175, 55, 0.2);">
                            ...
                            <option value="completa" {{ request('tipo_montura') == 'completa' ? 'selected' : '' }}>Completa</option>
                            <option value="semi_al_aire" {{ request('tipo_montura') == 'semi_al_aire' ? 'selected' : '' }}>Semi al aire</option>
                            <option value="al_aire" {{ request('tipo_montura') == 'al_aire' ? 'selected' : '' }}>Al aire</option>
                        </select>
                    </div>
                        ...
                        <div class="row g-2">
                            <div class="col-6">
                                <input type="number" class="form-control" name="precio_min" placeholder="Mínimo"
                                       value="{{ request('precio_min') }}" min="0" style="background: rgba(26, 26, 26, 0.6); border-color: rgba(212, 175, 55, 0.2);">
                            </div>
                            <div class="col-6">
                                <input type="number" class="form-control" name="precio_max" placeholder="Máximo"
                                       value="{{ request('precio_max') }}" min="0" style="background: rgba(26, 26, 26, 0.6); border-color: rgba(212, 175, 55, 0.2);">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <a href="{{ route('catalogo.index') }}" class="btn btn-outline-gold fw-semibold py-2">
